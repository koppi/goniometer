...
void getPulseDefaultSink(void *data) {
    ...
    pulseaudio_context = pa_context_new(mainloop_api, "goniometer device list");
...
void *input_pulse(void *data) {
    ...
    if (!(s = pa_simple_new(NULL, "goniometer", PA_STREAM_RECORD, audio->source, "audio for goniometer", &ss,
                            NULL, &pb, &error))) {
    ...
    while (!audio->terminate) {
        ...
        write_to_input_buffers(frames * channels, buf, data);
    }
